...
Logs decomposition events to .orch/logs/ for debugging and audit trail.
"""
...
def _parse_scenarios(content: str) -> list[dict[str, list[str]]]:
    ...
    for line in content.splitlines():
        ...
        for pat in _SCENARIO_PATTERNS:
            ...
            if m:
                ...
                new_scenario = cast(
                    dict[str, list[str]], {"title": title, "steps": [], "criteria": []}
                )
                ...
                tasks.append(
                    {
                        ...
                        "parent_id": None,
                        "root_spec_path": str(spec_path),
                        "ir_snippet": _build_edge_snippet(
                            edge_idx, edge_str, feature_context
                        ),
                    }
                )
...
def handle_decompose(args) -> dict:
    ...
    if spec_path.suffix.lower() == ".json":
        ...
        if not valid:
            ...
            error_msg = "Invalid spec.ir.json:\n" + "\n".join(error_lines[:10])
    ...
    # Extract feature IDs for logging
    feature_ids = [t["id"] for t in result_tasks[:5]]  # Log first 5
